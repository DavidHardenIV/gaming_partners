...
<head>
    <meta charset="UTF-8">
    <title>Find Apex Legends Teammates</title>
...
<main>
    <section class="game-form">
        <div class="container">
            <h1 class="game-title">Find Teammates for Apex Legends</h1>
            <form action="submit_waiting.php" method="POST">
                <input type="hidden" name="game" value="Apex Legends">

                <label for="apex_name">Enter Your Apex Username:</label>
                <input type="text" name="apex_name" placeholder="Enter your Apex username" required>
                ...
                <textarea name="additional_notes" placeholder="Legend, mode, mic, etc."></textarea>
    ...
    <section class="waiting-lobby">
        <div class="container">
            ...
            <table class="waiting-table">
                ...
                $stmt = $db->prepare("SELECT * FROM waiting_lobby WHERE game = 'Apex Legends' ORDER BY created_at DESC");
                $stmt->execute();
                $players = $stmt->fetchAll(PDO::FETCH_ASSOC);

                foreach ($players as $player): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($player['apex_name']); ?></td>
