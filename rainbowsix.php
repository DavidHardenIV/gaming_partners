...
<head>
    <meta charset="UTF-8">
    <title>Find Rainbow Six Siege Teammates</title>
...
<main>
    <section class="game-form">
        <div class="container">
            <h1 class="game-title">Find Teammates for Rainbow Six Siege</h1>
            <form action="submit_waiting.php" method="POST">
                <input type="hidden" name="game" value="Rainbow Six Siege">

                <label for="r6_name">Enter Your R6 Name:</label>
                <input type="text" name="r6_name" placeholder="Enter your Ubisoft username" required>
                ...
                <textarea name="additional_notes" placeholder="Attack/Defend, mic, timezone, etc."></textarea>
    ...
    <section class="waiting-lobby">
        <div class="container">
            ...
            <table class="waiting-table">
                ...
                $stmt = $db->prepare("SELECT * FROM waiting_lobby WHERE game = 'Rainbow Six Siege' ORDER BY created_at DESC");
                $stmt->execute();
                $players = $stmt->fetchAll(PDO::FETCH_ASSOC);

                foreach ($players as $player): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($player['r6_name']); ?></td>
